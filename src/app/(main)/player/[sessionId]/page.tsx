...
export default function PlayerPage() {
  ...
  useEffect(() => {
    getSessionDetails(sessionId)
      ...
      .catch(() => setError("Failed to load session data. Please try again later."));
  }, [sessionId]);
  ...
  if (error) return <div className="max-w-md mx-auto text-center p-8 bg-red-50 border border-red-200 rounded-lg text-red-700">{error}</div>;
